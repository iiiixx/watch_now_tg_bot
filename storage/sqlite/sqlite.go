...
// Save save given page to storage.
...
// Remove remove page from storage.
...
// IsExists check if page exists in storage.
func (s *Storage) IsExists(ctx context.Context, page *storage.Page) (bool, error) {
	...
	if err := s.db.QueryRowContext(ctx, q, page.URL, page.UserName).Scan(&count); err != nil {
		return false, fmt.Errorf("can't check existance of the page: %w", err)
	}

	return count > 0, nil
}

// Init initi if page exists in storage.
